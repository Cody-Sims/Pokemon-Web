import { MapDefinition, parseMap } from '../shared';

// Scalecrest Citadel — Town 8 (24×25). Gym 7 (Dragon — Drake)
// Ancient dragon fortress with cliff overlook, dragon scale courtyard
// Æ=FORTRESS_WALL  Ð=DRAGON_SCALE_FLOOR  ð=DRAGON_STATUE  ^=CLIFF_FACE
const scalecrestGround = parseMap([
  'ÆÆÆÆÆÆÆÆÆð.PP.ðÆÆÆÆÆÆÆÆÆ',
  'Æ..........PP.........^Æ',
  'Æ.CCCCCCC..PP.RRRRRRR.^Æ',
  'Æ.c$ccccc..PP.HH&HHHH.^Æ',
  'Æ.ccecccc..PP.HHHDHHH.^Æ',
  'Æ.ð..PP.PPPPPP..ð.....^Æ',
  'Æ....PP.ÐÐ.....ÐÐ.....^Æ',
  'Æ..ð.PP..MMMMMM.ð.....^Æ',
  'Æ....PP..mm&mmm.......^Æ',
  'Æ....PP..mmnmmm.....ð.^Æ',
  'Æ....ÐÐÐPPPPPPPPÐÐÐ...^Æ',
  'Æ....ÐÐÐ....ð..ÐÐÐ....^Æ',
  'Æ.ð..ÐÐÐ.......ÐÐÐ....^Æ',
  'Æ....ÐÐÐ.......ÐÐÐ....^Æ',
  'Æ....ÐÐÐPPPPPPPPÐÐÐ...^Æ',
  'Æ.ð..PP...........ð...^Æ',
  'Æ..........PP..AAAAAAA^Æ',
  'Æ..........PP..ggg&ggg^Æ',
  'Æ..ð.......PP..gggaggg^Æ',
  'Æ..........PP.........^Æ',
  'Æ.ð........PP......ð..^Æ',
  'Æ..........PP..........Æ',
  'Æ.......ð..PP..ð.......Æ',
  'Æ..........PP..........Æ',
  'ÆÆÆÆÆÆÆÆÆð.PP.ðÆÆÆÆÆÆÆÆÆ',
]);

export const scalecrestCitadel: MapDefinition = {
  key: 'scalecrest-citadel', width: 24, height: 25, ground: scalecrestGround,
  encounterTableKey: '', battleBg: 'bg-citadel', displayName: 'Scalecrest Citadel',
  npcs: [
    { id: 'scalecrest-sign', tileX: 13, tileY: 1, textureKey: 'generic-trainer', facing: 'down',
      dialogue: ['SCALECREST CITADEL', '"Ancestral Fortress of the Dragon Keepers"'] },
    { id: 'scalecrest-npc', tileX: 16, tileY: 6, textureKey: 'npc-male-5', facing: 'left',
      dialogue: ['Drake is our Gym Leader. His dragons descend from ancient guardians.', 'Only the worthy may pass!'] },
    { id: 'tutor-scalecrest', tileX: 8, tileY: 13, textureKey: 'npc-male-5', facing: 'right',
      dialogue: ['Dragon Tutor: I teach the ancient dragon arts.', 'Dragon Tutor: Bring me Heart Scales and I shall share my knowledge.'],
      interactionType: 'move-tutor', interactionData: 'tutor-scalecrest' },
    { id: 'scalecrest-wren', tileX: 16, tileY: 15, textureKey: 'npc-male-2', facing: 'left',
      dialogue: ['Dragon Keeper Wren: A Dragonair in the caves is in pain...',
        'Wren: The Aether disturbance is affecting it terribly.',
        'Wren: If you could find an Aether Salve, it might help.',
        'Wren: The ingredients are in Verdantia and Ember Mines.'],
      setsFlag: 'quest_dragonLament_started' },
    // ─── Veteran Knox (citadel gate, gives Scope Lens) ───
    { id: 'scalecrest-knox', tileX: 8, tileY: 23, textureKey: 'npc-ace-trainer', facing: 'right',
      dialogue: [
        'Veteran Knox: I\'ve stood guard at this citadel for thirty years.',
        'Knox: Back in my day, we fought the first Aether surge...',
        'Knox: Many good trainers were lost that day.',
        'Knox: Here, take this Scope Lens. It served me well.',
        'Knox: May it help you see clearly in the battles ahead.',
      ],
      setsFlag: 'received_scope_lens',
      givesItem: 'scope-lens',
      flagDialogue: [
        {
          flag: 'received_scope_lens',
          dialogue: [
            'Veteran Knox: The Scope Lens sharpens your focus.',
            'Knox: Critical hits can turn the tide of any battle!',
          ],
        },
      ],
    },
  ],
  trainers: [],
  warps: [
    { tileX: 11, tileY: 0, targetMap: 'route-7', targetSpawnId: 'from-scalecrest' },
    { tileX: 12, tileY: 0, targetMap: 'route-7', targetSpawnId: 'from-scalecrest' },
    { tileX: 11, tileY: 24, targetMap: 'cinderfall-town', targetSpawnId: 'from-scalecrest' },
    { tileX: 12, tileY: 24, targetMap: 'cinderfall-town', targetSpawnId: 'from-scalecrest' },
    { tileX: 4, tileY: 4, targetMap: 'scalecrest-pokecenter', targetSpawnId: 'default' },
    { tileX: 11, tileY: 9, targetMap: 'scalecrest-pokemart', targetSpawnId: 'default' },
    { tileX: 18, tileY: 18, targetMap: 'scalecrest-gym', targetSpawnId: 'default' },
    // House interior
    { tileX: 17, tileY: 4, targetMap: 'scalecrest-citadel-house-1', targetSpawnId: 'default' },
  ],
  spawnPoints: {
    'default': { x: 12, y: 12, direction: 'up' },
    'from-route-7': { x: 12, y: 1, direction: 'down' },
    'from-cinderfall': { x: 12, y: 23, direction: 'up' },
    'from-pokecenter': { x: 4, y: 5, direction: 'down' },
    'from-pokemart': { x: 11, y: 10, direction: 'down' },
    'from-gym': { x: 18, y: 19, direction: 'down' },
    'from-house-1': { x: 17, y: 5, direction: 'down' },
  },
};

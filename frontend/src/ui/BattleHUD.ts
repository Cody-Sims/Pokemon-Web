...
export class BattleHUD {
  ...
  constructor(
    ...
    this.nameText = scene.add.text(x, y, name, { fontSize: '16px', color: '#ffffff', fontStyle: 'bold' });
    this.levelText = scene.add.text(x + 140, y, `Lv${level}`, { fontSize: '14px', color: '#ffffff' });
    this.hpBar = new HealthBar(scene, x, y + 22, 180, 10, maxHp);
    this.hpBar.setValue(currentHp);
    this.hpText = scene.add.text(x + 90, y + 34, `${currentHp}/${maxHp}`, { fontSize: '12px', color: '#ffffff' }).setOrigin(0.5, 0);
  }

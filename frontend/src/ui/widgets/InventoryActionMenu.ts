...
import { COLORS, FONTS, PANEL_PRESETS, RADII, SPACING, mobileFontSize } from '@ui/theme';
import {
  SelectableController,
  type SelectableControllerConfig,
} from '@ui/controls/SelectableController';
...
export class InventoryActionMenu {
  ...
  constructor(
    ...
    private readonly config: InventoryActionMenuConfig,
  ) {
    const layout = ui(scene);
    const panelX = Math.min(Math.max(layout.cx, 140), layout.w - 80);
    this.panel = new NinePatchPanel(
      scene,
      panelX,
      layout.cy,
      148,
      config.labels.length * 34 + SPACING.lg,
      {
        ...PANEL_PRESETS.choice,
        cornerRadius: RADII.md,
      },
    );
    this.texts = config.labels.map((label, index) =>
      scene.add
        .text(panelX, layout.cy - (config.labels.length - 1) * 17 + index * 34, label, {
          ...FONTS.body,
  ...
  private setSelectedIndex(index: number): void {
    this.texts.forEach((text, textIndex) =>
      text.setColor(textIndex === index ? COLORS.textHighlight : COLORS.textWhite),
    );
  }
}

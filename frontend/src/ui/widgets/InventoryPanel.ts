...
import { COLORS, FONTS, PANEL_PRESETS, RADII, SPACING, isMobile, mobileFontSize } from '@ui/theme';
...
export class InventoryPanel {
  ...
  constructor(
    ...
    private readonly config: InventoryPanelConfig,
  ) {
    this.render();
  }
  ...
  private render(): void {
    ...
    this.objects.push(
      this.scene.add.rectangle(layout.cx, layout.cy, layout.w, layout.h, COLORS.bgDark),
    );
    this.panels.push(
      new NinePatchPanel(
        this.scene,
    ...
    this.objects.push(
      this.scene.add.rectangle(layout.cx, SPACING.xl + 20, 160, 2, COLORS.borderHighlight, 0.55),
    );

    const topCloseBtn = this.scene.add
      .text(layout.w - 16, 16, '✕', {
        ...FONTS.heading,
    ...
    this.objects.push(
      this.scene.add.rectangle(layout.cx, 78, layout.w - 40, 1, COLORS.border, 0.4),
    );
  ...
  private renderTabs(isPortrait: boolean, width: number): void {
    ...
    this.config.categories.forEach((cat, index) => {
      ...
      const text = this.scene.add
        .text(tabPad + tabSlotW * index + tabSlotW / 2, 58, label, {
          ...FONTS.bodySmall,
          fontSize: mobileFontSize(isPortrait ? 11 : 12),
        })
        .setOrigin(0.5, 0)
        .setInteractive({ useHandCursor: true });
  ...
  private renderDetailBacking(isPortrait: boolean, layout: ReturnType<typeof ui>): void {
    ...
    this.panels.push(
      new NinePatchPanel(this.scene, layout.w - 160, layout.cy + 30, 280, layout.h - 160, {
        fillColor: COLORS.bgCard,
        fillAlpha: 0.7,
        borderColor: COLORS.border,
        cornerRadius: RADII.md,
  ...
  private renderFooter(isPortrait: boolean, layout: ReturnType<typeof ui>): void {
    ...
    this.objects.push(
      this.scene.add.text(
        isPortrait ? 16 : layout.w - 280,
        moneyY,
        formatMoney(this.config.money, { useGrouping: false }),
        {
          ...FONTS.body,
          color: COLORS.textHighlight,
        },
    ...
    if (isMobile()) {
      ...
      this.objects.push(
        this.scene.add.text(layout.cx, closeY, 'ESC to close', FONTS.caption).setOrigin(0.5),
      );
    }
  }
}

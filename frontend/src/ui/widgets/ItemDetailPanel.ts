...
export class ItemDetailPanel {
  ...
  render(entry: InventoryEntry | undefined): void {
    ...
    const x = isPortrait ? SPACING.xl : layout.w - 280;
    const wrapW = isPortrait ? layout.w - SPACING.xl * 2 : 240;
    let y = isPortrait ? 90 + Math.floor((layout.h - 130) * 0.55) + 24 : 110;

    this.group.add(
      this.scene.add.text(x, y, entry.item.name, {
        ...FONTS.body,
        fontStyle: 'bold',
    ...
    if (effectText) {
      this.group.add(
        this.scene.add.text(x, y, effectText, {
          ...FONTS.caption,
...
function effectSummary(entry: InventoryEntry): string {
  ...
  if (effect.type === 'heal-hp')
    return effect.amount === -1 ? 'Revives to half HP' : `Heals ${effect.amount} HP`;

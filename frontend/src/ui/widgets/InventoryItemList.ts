...
export class InventoryItemList {
  ...
  constructor(private readonly scene: Phaser.Scene, private readonly config: InventoryItemListConfig) {
  ...
  render(state: InventoryItemListRenderState): void {
    ...
    if (state.items.length === 0) {
      ...
      const empty = this.scene.add.text(layout.cx, layout.cy, 'No items', {
        ...FONTS.bodySmall,
    ...
    for (let index = state.scrollOffset; index < endIndex; index++) {
      ...
      const text = this.scene.add.text(30, y, entry.item.name, {
        ...FONTS.body,
        fontSize: mobileFontSize(15),
        color: index === state.selectedIndex ? COLORS.textHighlight : COLORS.textWhite,
      }).setInteractive({ useHandCursor: true });
      const qty = this.scene.add.text(qtyX, y, `x${entry.qty}`, FONTS.bodySmall).setOrigin(qtyOrigin, 0);
    ...
    if (state.scrollOffset > 0) {
      this.group.add(this.scene.add.text(150, startY - 16, '▲', {
        ...FONTS.caption,
        color: COLORS.textHighlight,
      }).setOrigin(0.5));
    }
    if (endIndex < state.items.length) {
      this.group.add(this.scene.add.text(150, startY + this.config.maxVisible * itemH, '▼', {
        ...FONTS.caption,
        color: COLORS.textHighlight,
      }).setOrigin(0.5));
    }

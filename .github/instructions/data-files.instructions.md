...
2. Preserve IDs as public lookup contracts; do not rename a key for consistency
   until every runtime lookup path is traced.
3. Use existing typed IDs such as `MoveId` and `MapKey` where available.
4. Use selectors from `@data/selectors` for runtime lookups instead of fallback
   labels or duplicated lookup logic.
5. Add to the appropriate per-type or per-category file.
6. Update the barrel `index.ts` if adding a new file.
7. Run `npm run build` to type-check.
8. Run `npm run test` to exercise integrity checks for learnsets, encounters,
   evolutions, trainer parties, warps, spawn IDs, item balls, and shop keys.

...
- **Settings screen cut off at the bottom on mobile portrait**: With 17 setting rows + 1 fullscreen row at `rowH=30`, the back/export/import buttons rendered behind the DOM touch controls overlay (~140 px tall on phones). [SettingsScene.ts](frontend/src/scenes/menu/SettingsScene.ts) now reserves an extra 150 px bottom-pad on mobile portrait so rows shrink first and the back row anchors above the home indicator. The `[ Export ] [ Back ] [ Import ]` row was also hard-coded to `cx ± 90/140 px`, which clipped on narrow viewports — it now clamps to `min(90/140, layout.w/2 - 56)`.
- **Pause menu items spilled past the panel in landscape mobile (HALL OF FAM, TIONS clipped)**: Items were placed at `panelX + 10` with `setOrigin(0.5)`, biasing the text right of the panel center, and the panel itself sat at the legacy 20 px right margin where the DOM A/B button column overlapped it. [MenuScene.ts](frontend/src/scenes/menu/MenuScene.ts) now centers items on `panelX` (no offset), enforces a 180 px minimum panel width so HALL OF FAME fits with padding, and uses a new `computeRightInset()` helper that pulls the menu 140 px in from the right edge on landscape mobile to clear the touch-control side panels.
- **Touch buttons stayed visible over menus / dialogues**: The pause menu, dialogue, settings, etc. ran on top of `OverworldScene` but the joystick, A/B buttons, and hamburger DOM/canvas overlays kept rendering, occluding HALL OF FAME and OPTIONS in landscape. [OverworldScene.ts](frontend/src/scenes/overworld/OverworldScene.ts) now hides the touch controls (`setVisible(false)`) on the scene `pause` event and shows them again on `resume`, so any overlay scene that pauses the overworld owns the full viewport.
- **Pier still cut off in mobile portrait**: The previous fix used a fixed -90 px follow offset that didn't scale with viewport height. [OverworldScene.ts](frontend/src/scenes/overworld/OverworldScene.ts) now uses `-Math.round(layout.h * 0.18)` so the player sits in roughly the upper 35% of the canvas, leaving the bottom 60% for what's south of them. The bottom camera bounds pad is also stretched purely downward (no longer split across both axes), so the camera can scroll past the southern map edge by the full DOM-control height.
- **Hamburger menu sat too low in the top-right**: Repositioned in [TouchControls.layoutMenuButton](frontend/src/ui/controls/TouchControls.ts) — same orientation in portrait + landscape, pinned 4 px below the safe-area inset and 8 px in from the right (was 8/12 px depending on orientation). The button is now within easy thumb reach without conflicting with the location HUD strip.
- **Minimap overlapped the action buttons in landscape mobile**: Was anchored bottom-right with a `130 px` lift to clear the buttons, but on phone landscape the lift wasn't enough and the minimap ended up adjacent to the A/B column. [MinimapScene.computePosition](frontend/src/scenes/menu/MinimapScene.ts) now pins the minimap to the safe top-left band (after the joystick column) on landscape mobile, matching the portrait location-aware anchor. Desktop landscape keeps the legacy bottom-right corner.
...
- **NPC interact prompt showed wrong key**: The floating tag above NPCs read `Z`, but `InputManager` only binds `SPACE`/`ENTER` to confirm — `Z` does nothing. Updated the desktop label to `SPACE` in [OverworldScene.updateInteractPrompt](frontend/src/scenes/overworld/OverworldScene.ts), and aligned all desktop hints in [hint-text.ts](frontend/src/utils/hint-text.ts) (`Press Z` → `Press SPACE`, `Press X` → `Press ESC`, `Enter` → `SPACE/ESC` as appropriate). [index.html](frontend/index.html) desktop key strip now lists `Space / Esc / B` instead of the obsolete `Z / X / Enter`.
- **Pause menu cut off on mobile portrait**: The 12-item side menu used `rowH = 48 * MOBILE_SCALE` (≈65 px on phones), producing an 812 px tall panel that overflowed an 800 px viewport. [MenuScene.ts](frontend/src/scenes/menu/MenuScene.ts) now derives `rowH`, `panelW`, `panelH`, and `fontPx` from the viewport via a new `computePanelDims()` helper that reserves 56 px top + 48 px bottom safe space, shrinks the row pitch (28-48 px) before dropping the font (14-18 px), and clamps the panel width so it never spills off narrow portrait screens. The same helper is reused inside `layoutOn(...)` so rotating with the menu open re-fits cleanly.
- **Page background flashed purple in some browsers/preview shells**: The body bg was already `#000000` but some embed-style preview environments overrode it with their own theme. Reinforced [index.html](frontend/index.html) with `background: #000000 !important; background-color: #000000 !important;` on `html`/`body` (both default and the `(hover: hover)` desktop media query) plus an explicit `background: #000000` on `canvas`.
- **Minimap & HUD overlays drew on top of dialogue**: `MinimapScene`, `QuestTrackerScene`, and `PartyQuickViewScene` are registered after `DialogueScene` in [game-config.ts](frontend/src/config/game-config.ts), so they layered above the dialogue panel. [DialogueScene.ts](frontend/src/scenes/overworld/DialogueScene.ts) now sleeps those three HUD scenes inside `create()` (via the new `hideHudOverlays()` helper) and wakes them again from `shutdown()` (`showHudOverlays()`), so dialogue boxes always cover the minimap/quest tracker/party row while text is on screen.
- **Camera stopped following near south map edge (Pallet Town pier hidden)**: Two compounding issues — (1) the deadzone was `viewportW * 0.2 × viewportH * 0.2`, which on mobile portrait (~400×800) meant a 160 px deadzone equal to the *entire* available scroll range, and (2) the touch controls overlay covered the bottom ~140 px of the canvas with no compensating bounds-pad. [OverworldScene.ts](frontend/src/scenes/overworld/OverworldScene.ts) now uses a fixed 1-tile deadzone (`TILE_SIZE × TILE_SIZE`) so the camera follows aggressively, and pads the bottom of the camera bounds by the touch-control height in mobile portrait so the player can stand on the southernmost pier tile without being hidden behind the A/B buttons.

---

## [2026-04-26]

### Added — Challenge runs + JSON save export/import (plan.md B.4 / D.6)

- **Challenge Run Presets (plan.md B.4)**: Four toggleable rule layers shipped — Monotype, Solo Run, No Items, Minimal Catches — defined in new [frontend/src/data/challenge-modes.ts](frontend/src/data/challenge-modes.ts). Centralized rule gates in [frontend/src/systems/engine/ChallengeRules.ts](frontend/src/systems/engine/ChallengeRules.ts) (`blockReasonForCatch`, `blockReasonForPartyAdd`, `blockReasonForItemUse`). [PlayerStateManager](frontend/src/managers/PlayerStateManager.ts) persists `challengeModes[]` and `monotypeLock` (serialized + restored). UI: [TitleScene.showChallengeSelect](frontend/src/scenes/title/TitleScene.ts) presents a multi-select after difficulty (toggle with SPACE / tap, ENTER to begin). Wired into [BattleCatchHandler](frontend/src/scenes/battle/BattleCatchHandler.ts) (catch gate), [InventoryScene](frontend/src/scenes/menu/InventoryScene.ts) (item-use gate), and [StarterSelectScene](frontend/src/scenes/pokemon/StarterSelectScene.ts) (locks the monotype to the starter's primary type). Verified by [tests/unit/systems/challenge-rules.test.ts](tests/unit/systems/challenge-rules.test.ts).
- **JSON Save Export/Import (plan.md D.6)**: New `SaveManager.exportJson()` / `downloadJson()` / `importJson()` in [SaveManager.ts](frontend/src/managers/SaveManager.ts). `importJson()` validates JSON shape, required fields (`playerName`, `party`, `badges`, `flags`), and that the version is not from the future, then writes to localStorage and re-applies via `loadAndApply()`. New `[ Export ]` and `[ Import ]` buttons flank the Back button in [SettingsScene](frontend/src/scenes/menu/SettingsScene.ts) (uses a hidden `<input type=file>` for picking, returns to TitleScene after a successful import). Verified by [tests/integration/managers/save-export-import.test.ts](tests/integration/managers/save-export-import.test.ts).

### Added — Tier-A gameplay depth (plan.md A.4 / A.5 / B.3)

- **Move pool expansion (plan.md A.4)**: 16 new moves added across underrepresented types in [frontend/src/data/moves/](frontend/src/data/moves/) — Bug (X-Scissor, Signal Beam, Fury Cutter, Lunge, Megahorn), Rock (Power Gem, Head Smash, Accelerock, Rock Polish, Ancient Power), Electric (Wild Charge, Electroweb), Ice (Icicle Crash, Frost Breath), Steel (Flash Cannon), Ghost (Shadow Claw). All carry effect metadata (priority, recoil, weather, multi-hit, stat-change). New tests in [tests/unit/data/move-pool-expansion.test.ts](tests/unit/data/move-pool-expansion.test.ts).
- **Held-item expansion (plan.md A.5)**: Added Persim Berry confusion cure (and routed Lum Berry through the same volatile-cure path) plus the four weather rocks (Heat / Damp / Smooth / Icy) in [frontend/src/data/item-data.ts](frontend/src/data/item-data.ts). New `HeldItemHandler.onVolatileApplied()` is invoked from `StatusEffectHandler` when confusion is applied; `HeldItemHandler.getWeatherDurationBonus()` is consumed by `MoveExecutor` so weather-setting moves run 8 turns instead of 5 when the user holds the matching rock. Verified by [tests/unit/battle/held-item-extended.test.ts](tests/unit/battle/held-item-extended.test.ts).
- **Speed-run timer + splits (plan.md B.3)**: New `SpeedrunSplit` model in [PlayerStateManager.ts](frontend/src/managers/PlayerStateManager.ts) (serialized + restored from save). [GameManager.addBadge](frontend/src/managers/GameManager.ts) and `addHallOfFameEntry` auto-record idempotent splits (`badge:<id>` + final `champion`). New `Show Speed-Run Timer` toggle in [SettingsScene](frontend/src/scenes/menu/SettingsScene.ts); when enabled, [OverworldScene](frontend/src/scenes/overworld/OverworldScene.ts) renders a live `H:MM:SS` overlay below the clock. Splits list rendered in [StatisticsScene](frontend/src/scenes/menu/StatisticsScene.ts) under the existing stats table. Verified by [tests/integration/managers/speedrun-splits.test.ts](tests/integration/managers/speedrun-splits.test.ts).

### Fixed — Mobile UI portrait/landscape pass

- **Hamburger menu unresponsive in portrait**: The in-canvas menu button used `setInteractive` on a circle nested inside two containers, which is unreliable on iOS Safari. Replaced with a direct canvas touch handler (matching how the A/B buttons are wired) in [TouchControls.ts](frontend/src/ui/controls/TouchControls.ts), widened the hit margin to +16px, and pushed the button further down (72px from top) so it clears both the QuestTracker strip and iOS top-edge gesture areas.
- **Canvas not resizing on rotation**: Strengthened orientation handling in [main.ts](frontend/src/main.ts) by adding listeners on `screen.orientation.change`, `visualViewport.resize`, and `matchMedia('(orientation: portrait)')` plus a five-shot delayed resize sequence (0/60/200/500/1000 ms) to catch iOS Safari's lagged viewport reporting after rotation.
- **Minimap covering action buttons in portrait**: Moved the minimap to the top-left corner in portrait orientation (the user's suggested location, free of clock/HUD/QuestTracker/buttons). Landscape and desktop keep the legacy bottom-right placement. Updated [MinimapScene.ts](frontend/src/scenes/menu/MinimapScene.ts) `computePosition`.
- **IntroScene text clipped at bottom of viewport**: The intro slides anchored their text at `y = h * 0.72` with downward growth, so 4-line slides ran off the bottom of iPhone viewports (where iOS Safari's chrome overlaps the canvas). [IntroScene.ts](frontend/src/scenes/title/IntroScene.ts) now anchors slide text and the confirmation message from the bottom in portrait (origin `(0.5, 1)`, position `h - 80`), uses a slightly smaller mobile font, and wraps to 86% of canvas width.
- **Settings menu rows overlapping in portrait**: Labels rendered on top of values because the layout assumed desktop column widths (`label@100`, `value@w-150`, `arrows@w-95/w-210`). [SettingsScene.ts](frontend/src/scenes/menu/SettingsScene.ts) now computes a portrait-aware compact layout: labels at `x=24`, value/arrows hugged to the right edge, smaller font (14px), tighter row height auto-sized to fit all 18 rows in available height, smaller back button, and reduced panel margins.

### Added — Mobile UI follow-up (responsive layout + safe-area + e2e)

- **Settings menu now re-flows on rotation**: [SettingsScene.ts](frontend/src/scenes/menu/SettingsScene.ts) extracted its layout into a `buildLayout()` method wrapped in `layoutOn(this, ...)`. Every layout-derived child lives on a `layoutLayer` container that gets destroyed and rebuilt when the viewport changes, with cursor position preserved across the rebuild. Rotating the device (or resizing the browser) while the settings menu is open now produces the correct portrait or landscape compact layout instead of leaving rows stuck in their previous orientation.
- **Safe-area inset awareness**: New `getGameSafeAreaInsets(camera)` helper in [safe-area.ts](frontend/src/utils/safe-area.ts) converts the canvas's CSS-pixel safe-area overflow into Phaser game-coord insets via the camera scale ratio. Consumed by [TouchControls.layoutMenuButton](frontend/src/ui/controls/TouchControls.ts) and [MinimapScene.computePosition](frontend/src/scenes/menu/MinimapScene.ts) so the hamburger button and minimap stay clear of iOS notches/home indicators on devices where the canvas overflows the body's safe-area padding box. [computeGameDimensions](frontend/src/utils/constants.ts) now prefers `document.body.clientWidth/clientHeight` (which respects the body's safe-area padding) over raw `window.innerWidth/innerHeight` so dimensions match the actual canvas-renderable area.
- **DialogueScene portrait polish**: The dialogue box was a fixed 100px tall, which clipped multi-line text in portrait where lines wrap more often (the screenshot showed "Come visit my lab when you're ready — I" with the next line cut off). [DialogueScene.ts](frontend/src/scenes/overworld/DialogueScene.ts) now uses 130px tall in portrait + a smaller 14px base font + matching speaker/portrait/advance-indicator anchoring through the existing `layoutOn` re-layout block.
- **Mobile UI E2E coverage**: New [tests/e2e/mobile-ui.spec.ts](tests/e2e/mobile-ui.spec.ts) covers (a) booting + opening settings + adjusting values on a 390×844 portrait viewport, (b) rotating between portrait and landscape without runtime errors, and (c) the DOM hamburger button being present and tappable. Three tests, all passing on the `mobile-chromium` project.
- **E2E config fix**: [playwright.config.ts](tests/e2e/playwright.config.ts) `webServer` block now sets `cwd: '../..'` so `npm run dev` actually launches from the project root. Without this, `npm run test:e2e` failed locally with `ENOENT: tests/package.json` because Playwright was invoking npm from the config directory instead of the workspace root.

### Added — Visual & UX Polish (plan.md Tier C — second pass)

- **10 new cutscenes (plan C.5)**: Authored `mother-farewell`, `first-badge-celebration`, `marina-first-meeting`, `rook-first-encounter`, `aldric-hologram`, `kael-tag-team`, `marina-rescue`, `aldric-inner-sanctum`, `league-arrival`, and `shattered-isles-arrival` in [frontend/src/data/cutscene-data.ts](frontend/src/data/cutscene-data.ts) (28 total). Wired three of them into existing maps via `triggerCutscene`: `aldric-hologram` on the Ironvale hologram object, `league-arrival` on the League Guide NPC, and `shattered-isles-arrival` on a new shore marker sign.
- **Battle platform widget (plan C.2 remainder)**: New [BattlePlatform](frontend/src/ui/widgets/BattlePlatform.ts) widget collapses the previous 4-stack of `add.ellipse()` calls into a single Graphics-based ringed pixel-art platform with built-in sprite drop shadow. Replaced both player and enemy platforms in [BattleScene](frontend/src/scenes/battle/BattleScene.ts) and added a biome-aware palette helper that swaps in cave / sand / volcanic / tech palettes based on the active battle background key.
- **Voltara City visual redesign (plan C.1 starter)**: [voltara-city.ts](frontend/src/data/maps/cities/voltara-city.ts) now centers a 4×2 Aether Conduit Pillar setpiece (`Ɖ`) flanked by `AETHER_CRYSTAL` accents (`÷`), with `GEAR` (`Ω`) and `PIPE` (`π`) decorations along the south entry corridor and asymmetric green pockets around the plaza. All warp / building / spawn-point coordinates preserved so existing routes and saves continue to work; default spawn moved to `(11, 17)` so first-visit players face the new pillar.
- **Aurum Pixel BMFont (plan C.4 foundation)**: New [generate-pixel-font.js](frontend/scripts/generate-pixel-font.js) builds a 95-glyph BMFont (PNG glyph atlas + XML descriptor) from an embedded 5×7 pixel font source. Outputs to [frontend/public/assets/fonts/aurum-pixel.png](frontend/public/assets/fonts/aurum-pixel.png) + `aurum-pixel.xml`. Loaded in [PreloadScene](frontend/src/scenes/boot/PreloadScene.ts) and exposed via a [PixelText](frontend/src/ui/widgets/PixelText.ts) wrapper. Adopted by the Overworld HUD location label as a proof-of-concept (with a `cache.bitmapFont.exists()` guard so test harnesses that skip preload still render via the system font).
- **Cutscene + new test coverage**: Added 3 cutscene-integrity tests (every cutscene has actions, snake_case flags, and the 10 new cutscenes are present) bringing the visual polish suite to 17 tests / 2,040 total.

### Added — Visual & UX Polish (plan.md Tier C)

- **Glow emitter system**: New `frontend/src/systems/rendering/GlowEmitterSystem.ts` adds soft additive-blend pulse overlays for three tile categories: aether crystals (slow cyan pulse), Voltara tech tiles (CONDUIT / ELECTRIC_PANEL / WIRE_FLOOR — fast amber pulse), and window family tiles (HOUSE_WINDOW / LAB_WINDOW / CENTER_WINDOW / WINDOW — warm glow that brightens at night via `GameClock`). Capped at 64 emitters per category per map. Hooked into `OverworldScene.update()` and properly destroyed in `shutdown()`.
- **Quest tracker HUD polish**: `QuestTrackerScene` now sports a gold accent stripe, a star icon, a darker plate background, and cross-fade animations when the active quest or step changes. The HUD fades out when no quest is active.
- **Battle bar frames**: New `frontend/src/ui/widgets/BarFrame.ts` widget draws a 1px outer ring + drop shadow + optional type-colored accent stripe around HP / EXP bars. Applied to single-battle player + enemy HP and EXP bars, and double-battle partner + second-enemy HP bars in `BattleScene`.
- **Battle move menu selection ring**: `BattleMoveMenu` draws a thicker, type-colored ring around the active move button (with a dark outer shadow for contrast) so the cursor is visually unambiguous on all backgrounds. Replaces the previous text-color-only highlight.
- **Status Master achievement wiring**: `BattleUIScene` now tracks per-battle counts of damaging vs status moves used by the player (`playerDamagingMovesUsed`, `playerStatusMovesUsed`). `BattleVictorySequence.showTrainerRewardsThenEnd()` unlocks `status-master` when at least one status move was used and zero damaging moves were used in the battle.
- **Region overview map generator** (plan.md C.7): New `temp/scripts/map-gen/region/region-map.ts` script auto-generates a 1600×1200 PPM image of the entire Aurum Region. It introspects every map source file, builds the directed warp graph, runs a deterministic seeded force-directed layout, and renders nodes (cities / routes / dungeons) with category-coded colors, labels, and warp edges. New npm script `map:region`. Add `--interiors` to include house/PC/mart interiors. Output: `temp/region-map.ppm`.
- **Visual polish unit tests**: 14 new tests in `tests/unit/systems/visual-polish.test.ts` cover glow tile categorisation, region map regex extractors, and the status-master predicate.
...
- **Path-specific instruction files**: Created domain-scoped `.instructions.md` files for battle (`frontend/src/battle/**`), data files (`frontend/src/data/**`), scenes (`frontend/src/scenes/**`), entities (`frontend/src/entities/**`), managers (`frontend/src/managers/**`), systems (`frontend/src/systems/**`), UI (`frontend/src/ui/**`), and tests (`tests/**`)
...
- **Systems subdirectory CONTEXT.md files**: Added navigational context to all 4 systems subdirectories (`audio/`, `overworld/`, `rendering/`, `engine/`) with file tables
